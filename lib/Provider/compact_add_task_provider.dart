...
/// CompactAddTaskProvider - Compact task ekleme için state management
/// AddTaskProvider'den daha basit ve hızlı
class CompactAddTaskProvider with ChangeNotifier {
  // Controllers
  ...
  Duration _taskDuration = const Duration(hours: 0, minutes: 0); // Duration için kalan süre
  ...
  bool get isNotificationOn => _notificationAlarmState == 1;
  bool get isAlarmOn => _notificationAlarmState == 2;

  CompactAddTaskProvider() {
    _selectedDate = DateTime.now();
    LogService.debug('🟢 CompactAddTaskProvider initialized');
  }
  ...
  /// Task duration güncelle (Duration için)
  void updateTaskDuration(Duration value) {
    _taskDuration = value;
    LogService.debug('⏱ Task duration updated: ${value.inMinutes}min');
    notifyListeners();
  }
  ...
  String? validateInputs() {
    ...
    if (name.isEmpty) {
      LogService.error('❌ CompactAdd validation failed: Task name is empty');
      return 'Task name cannot be empty';
    }

    if (name.length > 200) {
      LogService.error('❌ CompactAdd validation failed: Task name too long');
      return 'Task name is too long (max 200 characters)';
    }

    LogService.debug('✅ CompactAdd validation passed');
    return null;
  }
  ...
  TaskModel toTaskModel() {
    return TaskModel(
      ...
      remainingDuration: _selectedTaskType == TaskTypeEnum.TIMER ? _taskDuration : null,
      priority: _priority,
  ...
  void reset() {
    ...
    _targetCount = 1;
    _taskDuration = const Duration(hours: 0, minutes: 0);
    LogService.debug('🔄 CompactAddTaskProvider reset');
    notifyListeners();
  }
  ...
  void clear() {
    ...
    LogService.debug('🟡 CompactAddTaskProvider disposed');
  }
